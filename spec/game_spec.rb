...
module TicTacToe
  describe Game do
    ...
    context "#initialize" do
      it "randomly slects a current player" do
        Array.any_instance.stub(:shuffle) {[frank, bob]}
        game = Game.new([bob, frank])
        expect(game.current_player.name).to eq "frank"
      end
      
      it "randomly slects an other player" do
        Array.any_instance.stub(:shuffle) {[frank, bob]}
        game = Game.new([bob, frank])
        expect(game.other_player.name).to eq "bob"
      end
    end
    ...
    context "#switch_players" do
      ...
      it "switches @other_player and @current_player" do
        game = Game.new([bob, frank])
    ...
    context "#get_move" do
      it "converts the human_move of '1' to [0, 0]" do
        game = Game.new([bob, frank])
